...
from utils.utils import test, save_model, set_params, maybe_log_to_wandb, flower_parameters_size_bytes
...
def _build_traffic_metrics(
    ...
    fallback_full_model_size = flower_parameters_size_bytes(parameters)
...
class EvaluateLora:
    def __init__(
        ...
        device,
        args: Namespace,
        base_parameter_names=None,
        base_parameter_shapes=None,
    ):
        self.args = args
        eval_batch_size = int(getattr(args, "batch_size", 256))
        self.test_loader = DataLoader(
        ...
        self.model = model
        self.device = device
        self.base_parameter_names = list(base_parameter_names or [])
        self.base_parameter_shapes = [tuple(shape) for shape in (base_parameter_shapes or [])]
        self._peft_payload_initialized = False
        self.past_a_matrix, self.past_b_matrix = extract_AB_matrix(model.state_dict())

    def _plain_base_state_items(self):
        if not self.base_parameter_names or not self.base_parameter_shapes:
            raise ValueError("EvaluateLora requires plain base-model parameter metadata.")
        return list(zip(self.base_parameter_names, self.base_parameter_shapes))

    def _set_base_params(self, parameters) -> None:
        expected = self._plain_base_state_items()
        if parameters is None or len(parameters) != len(expected):
            raise ValueError(
                f"Expected base initialization payload with {len(expected)} tensors, "
                f"received {0 if parameters is None else len(parameters)}."
            )
        current_state = self.model.state_dict()
        updates = {}
        for idx, (name, expected_shape) in enumerate(expected):
            param = parameters[idx]
            if tuple(getattr(param, "shape", ())) != tuple(expected_shape):
                raise ValueError(
                    f"Base initialization tensor shape mismatch at index {idx} "
                    f"({name}): received {tuple(getattr(param, 'shape', ()))}, "
                    f"expected {tuple(expected_shape)}."
                )
            candidate_names = (name, f"base_model.model.{name}")
            target_name = next((key for key in candidate_names if key in current_state), None)
            if target_name is None:
                raise ValueError(f"Could not map base initialization tensor {name!r} into PEFT model.")
            updates[target_name] = torch.from_numpy(np.copy(param))
        current_state.update(updates)
        self.model.load_state_dict(current_state, strict=False)

    def _set_lora_for_phase(self, server_round, parameters) -> None:
        ...
        if received_tensors == expected_lora_tensors:
            ...
            self._peft_payload_initialized = True
            return
        if self._peft_payload_initialized:
            raise ValueError(
                "EvaluateLora expected a recurring PEFT payload after adapter aggregation, "
                f"received {received_tensors} tensors instead of {expected_lora_tensors}."
            )
        self._set_base_params(parameters)

    def __call__(self, server_round, parameters, config):
        if self.args.strategy in {"fedlora", "fedloha"}:
            self._set_lora_for_phase(server_round, parameters)
        else:
            set_params(self.model, parameters, self.args.fedbn)
        to_log = {}
