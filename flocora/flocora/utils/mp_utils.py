...
from utils.utils import (
    ...
    train,
    inst_model_info,
    compute_flower_parameters_size_bytes,
    flower_parameters_size_bytes,
    ndarrays_to_flower_parameters_payload,
...
def _base_state_cache_path(fl_info) -> Path:
    safe_exp = str(getattr(fl_info, "exp_name", "experiment")).replace("/", "_")
    safe_cid = str(getattr(fl_info, "cid", "client")).replace("/", "_")
    cache_dir = Path("/tmp/flocora_base_state_cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / f"{safe_exp}_{safe_cid}.npy"


def _save_base_state_cache(fl_info, parameters) -> None:
    np.save(_base_state_cache_path(fl_info), np.array(parameters, dtype=object), allow_pickle=True)


def _load_base_state_cache(fl_info):
    cache_path = _base_state_cache_path(fl_info)
    if not cache_path.exists():
        raise RuntimeError(
            "Missing cached base initialization payload for recurring FLoCoRA round. "
            "The frozen base model W_0 must be loaded from the initial broadcast, not "
            "silently recreated from random initialization."
        )
...
def mp_fit(info, fl_info,config, parameters, return_dict):
    ...
    try:
        ...
        received_parameters_for_accounting = parameters
        payload_phase = str(config.get("flocora_payload_phase", "")).lower()

        if fl_info.lora_config is not None and payload_phase == "base_initialization":
            ...
            _save_base_state_cache(fl_info, parameters)
            parameters = None
        elif fl_info.lora_config is not None and payload_phase == "peft_recurring":
            cached_base_parameters = _load_base_state_cache(fl_info)
            _validate_parameter_layout(
            ...
            set_params(net, cached_base_parameters,fedbn=info.fedbn)

        if fl_info.lora_config is not None :
            if payload_phase not in {"base_initialization", "peft_recurring"}:
                raise ValueError(f"Unknown FLoCoRA payload phase: {payload_phase!r}.")
        ...
        if parameters is not None:
            if fl_info.lora_config is not None:
                ...
                if payload_phase != "peft_recurring":
                    raise ValueError(
                        "FLoCoRA base_initialization parameters must be consumed before "
                        "LoRA injection and must not be loaded into a PEFT-wrapped model."
                    )
                if not received_lora_payload:
                    raise ValueError(
                        "FLoCoRA peft_recurring phase requires a PEFT payload matching "
                        "get_lora_state_items(net)."
                    )
        ...
        upload_traffic = compute_flower_parameters_size_bytes(params)
        if payload_phase == "base_initialization":
            download_traffic = 0.0
        else:
            download_traffic = flower_parameters_size_bytes(received_parameters_for_accounting)
        serialized_upload_parameters = ndarrays_to_flower_parameters_payload(params)
        serialized_download_parameters = (
            None
            if payload_phase == "base_initialization"
            else ndarrays_to_flower_parameters_payload(received_parameters_for_accounting)
        )
        serialization_flops = estimate_serialization_flops(serialized_upload_parameters)
        deserialization_flops = estimate_deserialization_flops(serialized_download_parameters)
