...
def filter_wandb_metrics(metrics: Mapping[str, float]) -> Dict[str, float]:
    """Return a new W&B payload containing only approved experiment metrics."""

    return {key: value for key, value in metrics.items() if key in WANDB_METRIC_ALLOWLIST}


def _log_wandb_metrics(metrics: Mapping[str, float], *, step: Optional[int] = None) -> bool:
    """Log the filtered W&B payload if any approved metrics are present."""
...
def ndarrays_to_flower_parameters_payload(ndarrays):
    """Convert ndarray payloads to Flower Parameters as transmitted by NumPyClient."""
    from flwr.common.parameter import ndarrays_to_parameters

    return ndarrays_to_parameters(ndarrays)


def serialized_flower_parameters_size_bytes(parameters) -> float:
    """Return canonical serialized Flower Parameters protobuf payload bytes."""
    ...
    try:
        ...
        from flwr.common.serde import parameters_to_proto_msg as parameters_to_proto

    return float(len(parameters_to_proto(parameters).SerializeToString()))


def flower_parameters_size_bytes(payload) -> float:
    """Return serialized Flower Parameters bytes for Parameters or ndarray lists."""
    if payload is None:
        return 0.0
    if hasattr(payload, "tensors") and hasattr(payload, "tensor_type"):
        return serialized_flower_parameters_size_bytes(payload)
    return serialized_flower_parameters_size_bytes(ndarrays_to_flower_parameters_payload(payload))


def compute_flower_parameters_size_bytes(ndarrays) -> float:
    """Return complete serialized tensor bytes in Flower Parameters representation."""
    return flower_parameters_size_bytes(ndarrays)


def get_tensor_parameters(model,fedbn=False):
    return ndarrays_to_flower_parameters_payload(
...
def maybe_log_to_wandb(metrics: Mapping[str, float], *, step: Optional[int] = None) -> None:
    ...
    _log_wandb_metrics(log_payload, step=step)
...
def tell_history(
    ...
    # Preserve the complete Flower history/test summary/local result output.
    # W&B filtering happens only after this local artifact is written.
    with open(path + file_name + ".npy", "wb") as f:
        np.save(f, infos)

    if args is not None and args.wandb and report:
        _log_wandb_metrics(report)
