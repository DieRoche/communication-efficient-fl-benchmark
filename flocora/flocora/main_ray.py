...
if __name__ == "__main__":
    ...
    if args.strategy == "fedavg":
        ...
        base_model_state_items = [
            (name, tensor)
            for name, tensor in server_model.state_dict().items()
            if not (args.fedbn and "bn" in name)
        ]
        base_model_params = [tensor.cpu().numpy() for _, tensor in base_model_state_items]
        base_parameter_names = [name for name, _ in base_model_state_items]
        base_parameter_shapes = [tuple(tensor.shape) for _, tensor in base_model_state_items]
        forbidden_base_tokens = ("lora_", "hada_", "modules_to_save")
        if any(any(token in name for token in forbidden_base_tokens) for name in base_parameter_names):
            raise RuntimeError("Base initialization payload unexpectedly contains PEFT tensors.")
        if len(base_model_params) != len(base_parameter_names):
            raise RuntimeError("Base initialization payload metadata does not match tensor list.")
        base_initial_parameters = ndarrays_to_flower_parameters_payload(base_model_params)
        full_model_size = flower_parameters_size_bytes(base_initial_parameters)
        ...
        flocora_payload_size = compute_flower_parameters_size_bytes(initial_lora_params)
        ...
        kwargs_dict["initial_parameters"] = base_initial_parameters
        ...
        kwargs_dict["evaluate_fn"] = EvaluateLora(server_model, lora_config, test_set, device, args, base_parameter_names, base_parameter_shapes)
